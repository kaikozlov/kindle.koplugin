...
-- Byte-exact KRDS fixture with updated_lpr (timestamp 1000) and lpr
-- (timestamp 1100) objects.
...
describe("HelperClient", function()
    ...
    describe("native position sync", function()
        ...
        it("reports exact sync only for a readable KRDS sidecar", function()
            local client = HelperClient:new(client_opts)
            assert.is_true(client:nativeProgressAvailable("B007N6JEII", kfx_path))

            local sidecar = assert(io.open(tmpdir .. "/book.sdr/book.yjf", "wb"))
            sidecar:write("not a KRDS store")
            sidecar:close()
