...
local MAP = {
    ...
    fragments = {
        ...
        },
        {
            ...
            anchors = {
                { p = "p", eid = 1141, pid = 27300, t = 9, nodes = { { p = "p", n = 1, c = 0, v = 9 } } },
            },
        },
    },
}
...
describe("PositionMap", function()
    ...
    it("translates an XPointer into native coordinates", function()
        local result, err = PositionMap.translate_xpointer(MAP, "/body/DocFragment/body/div/p/em/text().4")
    ...
    it("translates a tail text node through its parent element", function()
        local result, err = PositionMap.translate_xpointer(MAP, "/body/DocFragment/body/div/p/text()[2].3")
        assert.is_not_nil(result, err)
    ...
    it("uses the indexed spine fragment", function()
        local result, err = PositionMap.translate_xpointer(MAP, "/body/DocFragment[2]/body/p/text().4")
        assert.is_not_nil(result, err)
    ...
    it("rejects XPointers on unanchored elements", function()
        local result = PositionMap.translate_xpointer(MAP, "/body/DocFragment/body/p[2]/text().1")
        assert.is_nil(result)
    end)

    it("round-trips a native position back to the same XPointer", function()
        local forward = assert(PositionMap.translate_xpointer(MAP, "/body/DocFragment/body/div/p/em/text().4"))
    ...
    it("round-trips a tail node", function()
        local forward = assert(PositionMap.translate_xpointer(MAP, "/body/DocFragment/body/div/p/text()[2].3"))
