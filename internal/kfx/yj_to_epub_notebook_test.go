...
func TestScribeDensityMidpointUsesFloorDivision(t *testing.T) {
	...
	if got := pythonFloorDivInt(-1, PNG_SCALE_FACTOR); got != -1 {
		t.Fatalf("-1 // %d = %d, want -1", PNG_SCALE_FACTOR, got)
	}
	if got := pythonFloorDivInt(-9, PNG_SCALE_FACTOR); got != -2 {
		t.Fatalf("-9 // %d = %d, want -2", PNG_SCALE_FACTOR, got)
	}
}

func TestScribeStrokeRetainsFirstMinusOnePoint(t *testing.T) {
	// Python starts last_x/last_y at None, not a numeric sentinel. A genuine
	// first point at (-1,-1) must therefore survive duplicate-point removal.
	posData := []byte{0x01, 0x01, 0x02, 0x00, 0x00, 0x00, 0x45} // [0, 1]
	all100 := []byte{0x01, 0x01, 0x02, 0x00, 0x00, 0x00, 0x14, 0x64}
	content := map[string]interface{}{
		"nmdl.type": "nmdl.stroke", "nmdl.brush_type": 0, "nmdl.color": 0,
		"nmdl.stroke_bounds": []interface{}{-1, -1, 100, 100},
		"nmdl.thickness":     float64(50),
		"nmdl.stroke_points": map[string]interface{}{
			"nmdl.num_points":              2,
			"nmdl.position_x":              append([]byte(nil), posData...),
	...
	parent := &svgElement{Tag: "svg"}
	nc := &notebookContext{contentContext: "sentinel"}
	scribeNotebookStroke(nc, content, parent, "stroke")
	if nc.err != nil {
		t.Fatal(nc.err)
	}
	if len(parent.Children) != 1 {
	...
	if d != "M -1 -1 L 0 0" {
		t.Fatalf("stroke path = %q, want first (-1,-1) point retained", d)
	}
}
