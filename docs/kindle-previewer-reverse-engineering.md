...
#### Producer investigation: `page_regions` is not emitted by the bundled EPUB producer or normal IMGOPT stage

A substantially deeper producer search was completed after the initial renderer trace.

At the Java level, `PageRegions` / `page_regions` occurs only in `com.amazon.kaf.c.b`, the KAF property enum. `DocumentRegions` additionally occurs in `com.amazon.kaf.c.aB$o`, the KAF entity-type enum, but no Java conversion class references the `DocumentRegions` entity constant. In other words, the bundled `EpubToKFXConverter-4.0.jar` contains the vocabulary and type declarations, but no reachable Java EPUB producer that directly constructs either property/entity.

This static result was checked against a controlled fixture sweep. Seventeen successful `EpubAdapterApp` conversions spanning ordinary fixed layout, comic metadata, children fixed layout, guided-view-native metadata, magnify markup, ComiXology identifiers, and region-navigation variants produced **zero serialized occurrences** of:

```text
page_regions
DocumentRegions/document_regions
virtual_panel
zoom_panel
yj.guided_view
```

in the ordinary KDF graph unless the source exercised an older panel/zoom representation explicitly. The important distinction is that strings such as `yj_guided_view_native` found in some generated KDFs are conversion-capability metrics under `com.amazon.yjconversion`; they are not property 666 `yj.guided_view` book data.

The full Previewer `KFXGenApp` pipeline was then made to run successfully, including `IMGOPTIMISATION`. The working invocation requires a bare supported locale code such as `en` rather than `en-US`, empty output/temp directories, the `fc` directory as `-libDir`, and `DYLD_LIBRARY_PATH` containing both `fc/jre/lib/server` and `fc/lib` so the `KindleImageProcessor` child can load `@rpath/libjvm.dylib`. A representative command is:

```sh
FC="REFERENCE/Kindle Previewer 3.app/Contents/lib/fc"
rm -rf /tmp/kfxgen/run && mkdir -p /tmp/kfxgen/run/out /tmp/kfxgen/run/tmp
DYLD_LIBRARY_PATH="$FC/jre/lib/server:$FC/lib" \
  "$FC/jre/bin/java" \
  -Djava.library.path="$FC/lib" \
  -cp "$FC/lib/*" \
  com.amazon.kfxconverter.app.KFXGenApp \
  -inputFile /tmp/book.epub \
  -outDir /tmp/kfxgen/run/out \
  -tmpDir /tmp/kfxgen/run/tmp \
  -libDir "$FC" \
  -locale en \
  -clientType PREVIEW \
  -disableLocMapGeneration
...
The pipeline status records `CREATEYJ` and `IMGOPTIMISATION` success. `PREVIEW` mode also preserves a pristine `canonical_YJ/book/book.kdf`, giving a direct before/after pair for the image-optimization stage.

That A/B test was performed for both reflowable and fixed-layout image books. `KindleImageProcessor` changed image resources, image metadata, local conversion symbols, and several bookkeeping fragments, but **did not add `page_regions`**. Searches by literal name and by SID 852 in all plausible Ion encodings remained zero before and after optimization. For the two controlled books:

```text
reflowable:   KDF 29696 -> 37888 bytes, page_regions 0 -> 0
fixed-layout: KDF 21504 -> 29696 bytes, page_regions 0 -> 0
```

So property 852 is not generated by ordinary `CREATEYJ` or the normal `IMGOPTIMISATION` path in Previewer 3.106.

#### The closest bundled producer: ComiXology EPUB3 region navigation

There is, however, a real region/panel importer in the Java converter. `com.amazon.adapter.c.a.a.p()` is instrumented as `CreateFixedRegions` / `Html2YJSh.HasEpub3Panels` and recognizes a legacy ComiXology-flavored EPUB profile. Its gate requires:

1. an EPUB2-style `dc:identifier` with `opf:scheme="URL"` whose value begins with `http(s)://www.comixology.com/`;
2. a fixed-layout/image-only page set with no text-page flag; and
3. exactly one manifest item carrying `properties="data-nav"`.

The data-nav parser accepts a `region-based` navigation document of approximately this form:

```xml
<nav epub:type="region-based">
  <ol>
    <li>
      <a href="chapter.xhtml#xywh=PIXEL:40,40,720,500">
...
`xywh` accepts `PIXEL` or `PERCENT` coordinates and exactly four numeric values. The transform in `com.amazon.adapter.common.n.a.s` constructs zoom-panel geometry, a backing image container, and a `ZOOM_IN` action, then marks guided-view state. This is a genuine region producer path, but it targets the older `zoom_panel` / `zoom_target` / panel machinery rather than directly writing `page_regions`.

Synthetic image-only ComiXology fixtures reach the gate (`HasEpub3Panels=1`) but currently die with a `NullPointerException` inside `com.amazon.adapter.common.n.a.s.a` in the standalone harness. A paired text-page control does not enter the panel transform. This makes the path structurally confirmed but not yet end-to-end serialized in our harness.

#### Native document-region storage exists, but its writer is still external to the observed pipeline

`libshared.dylib` contains a typed native `yjsdk::DocumentRegionContainer` subsystem, including `DocumentRegionEntry`, creator/handler RTTI, and the JNI export `BookContent_removeNativeDocumentRegionContainer`. The main Previewer executable consumes `page_regions` at render time as documented above. What remains missing is an observed writer call that creates a `DocumentRegionContainer` or emits property 852 from EPUB input.

The evidence therefore supports a narrower conclusion than the earlier open-ended hypothesis:

```text
ordinary EPUB -> CREATEYJ                 no page_regions observed
ordinary IMGOPTIMISATION                  no page_regions observed
legacy CMX region-based-nav importer       zoom/panel producer, incomplete in harness
native KAF document-region storage         present
Previewer renderer page_regions consumer   present
actual page_regions writer                 not found in bundled reachable EPUB pipeline
```

The most likely remaining source is an Amazon authoring/delivery-side producer such as Comic Creator/KPF tooling or another internal document-region stage, but that remains a hypothesis until a captured file or writer call proves it.
...
- A full `KFXGenApp` run including `IMGOPTIMISATION` was recovered. Bare locales such as `en`, empty output/temp directories, and a `DYLD_LIBRARY_PATH` that exposes `fc/jre/lib/server` to `KindleImageProcessor` are required for a reliable macOS run.
- Controlled before/after KDF comparisons prove ordinary `CREATEYJ` and `IMGOPTIMISATION` do **not** emit property 852 for either reflowable or ordinary fixed-layout image books.
- The bundled Java producer contains a legacy ComiXology `region-based` data-nav importer gated by an EPUB2-style ComiXology URL identifier, image-only fixed pages, and exactly one `data-nav` manifest item. It builds zoom-panel/zoom-target geometry and guided-view state, but synthetic fixtures currently reach the gate and then fail inside the dormant panel transform before serialization.
- Native KAF contains `DocumentRegionContainer` storage/management types, while the Previewer renderer consumes `page_regions`; no reachable bundled EPUB writer for either representation has been observed.
...
1. Expand the controlled semantic matrix beyond the twelve current fixtures:
   ...
   - page spreads and guided-view/document regions beyond the now-characterized ordinary/CMX producer paths;
...
2. Finish the new shared-symbol tail investigation:
   - `page_regions` is now narrowed to a renderer/native-storage feature not emitted by ordinary `CREATEYJ` or `IMGOPTIMISATION`; pursue external/authoring-side evidence only when a KPF/retail/comic specimen or writer call is available;
