...
# CI layer caching: inside GitHub Actions (GITHUB_ACTIONS=true) builds pass
# the type=gha layer cache so unchanged Dockerfile.test and crypto-hook
# layers — including the QEMU-emulated apt/compile/test steps — are reused
# across runs. Every distinct build passes its own `scope`: a gha-scoped
# export overwrites any previous cache under the same scope
# (moby/buildkit#2885), so one shared scope would leave only the last
# build's cache. Locally the builder's own layer cache serves the same
# role, so no flags are added outside CI.
...
test-image:
    ...
    if [ -z "$(docker images -q '{{ _test_image }}' 2>/dev/null)" ]; then
        ...
        if [ "${GITHUB_ACTIONS:-}" = "true" ]; then
            cache="--cache-from type=gha,scope=test-image --cache-to type=gha,mode=max,scope=test-image"
        fi
...
test-drm-hook:
    ...
    build() {
        local target="$1"
        echo "Building ARM DRM hook target: $target"
        local cache=""
        if [ "${GITHUB_ACTIONS:-}" = "true" ]; then
            cache="--cache-from type=gha,scope=drm-$target --cache-to type=gha,mode=max,scope=drm-$target"
        fi
        # shellcheck disable=SC2086
        docker buildx build --platform linux/arm/v7 $cache --target "$target" -f .github/Dockerfile.crypto_hook .
    }
